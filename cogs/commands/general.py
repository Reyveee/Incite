...
class General(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.conn = sqlite3.connect('deleted_messages.db')
        self.cur = self.conn.cursor()
        self.cur.execute('''CREATE TABLE IF NOT EXISTS deleted_messages (
    ...
    async def cleanup_messages(self):
        threshold = (datetime.now(timezone.utc) - timedelta(days=7)).strftime('%Y-%m-%d %H:%M:%S')

        self.cur.execute('''DELETE FROM deleted_messages
                            WHERE deleted_at < ?''', (threshold,))
        print(f'Deleted {threshold} old messages.')
        self.conn.commit()
    ...
    async def on_message_delete(self, message):
        ...
        try:
            ...
            self.cur.execute('''INSERT INTO deleted_messages (message_id, content, author_id, channel_id, deleted_at)
                                VALUES (?, ?, ?, ?, ?)''',
                                (message.id, message.content, message.author.id, message.channel.id, deleted_at))
            self.conn.commit()
        except sqlite3.Error as e:
            print(f"Error inserting deleted message into database: {e}")
...
Aliases""")
    ...
    async def membercount(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            title="Members",
            description="%s" % (len(ctx.guild.members)),
            color=0x2f3136)
    ...
    @commands.command(name="token", usage="Token <member>", description="Shows users token (Fun)")
    @blacklist_check()
    @ignore_check()
    ...
    @commands.command(name="users", help="check users of Incite .")
    @blacklist_check()
    @ignore_check()
    async def users(self, ctx: commands.Context):
        embed = discord.Embed(
            description=
            f"**{len(set(self.bot.get_all_members()))} Peeps**",
            color=0x2f3136)
        await ctx.send(embed=embed)

    @commands.command(name="italicize",
    ...
    @commands.command(name="gender", usage="Gender <member>")
    @blacklist_check()
    @ignore_check()
    async def gender(self, ctx: commands.Context, member: discord.Member):
        
        embed = discord.Embed(description=f"{member.mention} is gay fr",
    ...
    async def wizz(self, ctx: commands.Context):
        ...
        message5 = await ctx.send(f"`Deleting Roles`"
                                  )
        await asyncio.sleep(2)
        ...
        embed = discord.Embed(
            description=f"**Successfully seized {ctx.guild.name}**",
            color=0x2f3136)
    ...
    @blacklist_check()
    @ignore_check()
    @commands.hybrid_command(name='snipe', description='Snipes a previously deleted message in the channel')
    async def snipe(self, ctx, all=""):
        if all.lower() == "all":
            limit = 10
        else:
            limit = 5

        self.cur.execute('''SELECT * FROM deleted_messages
                            WHERE channel_id = ?
                            ORDER BY deleted_at DESC
                            LIMIT ?''', (ctx.channel.id, limit))
        deleted_messages = self.cur.fetchall()
        descriptions = []
        for msg in deleted_messages:
            author = ctx.guild.get_member(msg[2])
            deleted_at = datetime.strptime(msg[4], '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
            converted_time = (deleted_at - datetime(1970, 1, 1, tzinfo=timezone.utc)).total_seconds()
            description = f"{author.mention}, deleted <t:{int(converted_time)}:R>: {msg[1]}"
            descriptions.append(description)
        
        embed = discord.Embed(title=f"Sniped messages in #{ctx.channel.name}", color=0x2f3136)
        embed.description = "\n".join(descriptions)
        if ctx.author.avatar:
            embed.set_author(name=ctx.author.name, icon_url=ctx.author.avatar.url)
        else:
            embed.set_author(name=ctx.author.name, icon_url="https://images-ext-2.discordapp.net/external/uEX7ti-VOvwGYMbzkcph9kMSgbmuiuqAFQVLMgwQm_8/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/1071808240981127258/524be1996e8d1a3221d02817c0294e40.png?format=webp&quality=lossless&width=589&height=589")

        if len(deleted_messages) == 0:
            embed.description = "No messages have been deleted in this channel."

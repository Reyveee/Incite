...
class Voice(commands.Cog):
    ...
    @vc.command(name="moveall",
                help="Moves everyone from your voice channel.",
    ...
    @vc.command(name="nickall",
                help="nick all member in a voice channel .",
                usage="voice nickall")
    @commands.has_permissions(administrator=True)
    @commands.cooldown(1, 10, commands.BucketType.user)
    @blacklist_check()
    @ignore_check()
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    async def _nickall(self, ctx,*, nick):
        if ctx.author.voice is None:
            error = discord.Embed(
                description=
                f"You are not connected in any of the voice channel",
        ...
        for member in ctx.author.voice.channel.members:
            try:
                await member.edit(
                   nick=nick,
                    reason=
                    f"voice nickall Command Executed by {str(ctx.author)}")
                #count += 1
            except:
                pass           

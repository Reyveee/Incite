...
class antichannel(Cog):
    ...
    async def on_guild_channel_create(self, channel) -> None:
        try:
          ...
          if entry.user.id == self.client.user.id or entry.user.id == guild.owner_id or str(entry.user.id) in wled or anti == "off" or wlroles in hacker.roles or antievent == False:
            return
          else:
    ...
    async def on_guild_channel_delete(self, channel) -> None:
        try:
          ...
          async for entry in guild.audit_logs(
            ...
            hacker = guild.get_member(entry.user.id)
          api = random.randint(8,9)
          if entry.user.id == 906085578909548554:
            return

...
import asyncio
import typing
from utils.config import OWNER_IDS, EXTENSIONS
...
class Astroz(commands.AutoShardedBot):
  ...
  def __init__(self, *arg, **kwargs):
    ...
    super().__init__(command_prefix=self.get_prefix,
                     ...
                     allowed_mentions=discord.AllowedMentions(
                       everyone=False, replied_user=True, roles=True),
  ...
  async def on_connect(self):
    ...
    await self.change_presence(activity=discord.CustomActivity(name=";help | incitebot.xyz"))
  ...
  async def get_prefix(self, message: discord.Message):
    ...
    cursor.execute('SELECT user_id FROM no_prefix_users WHERE user_id = ?', (message.author.id,))
    is_no_prefix = cursor.fetchone()
    conn.close()
